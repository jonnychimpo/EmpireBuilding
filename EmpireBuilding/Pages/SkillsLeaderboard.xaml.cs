...
{
    public partial class SkillsLeaderboard : PhoneApplicationPage
    {
        int TheOption = 0;
        ...
        private void pivotLeaderBoards_LoadingPivotItem(object sender, PivotItemEventArgs e)
        {
            ...
            if (e.Item == pivotExplore)
            {
                TheField = "ExploreXP";
                FieldFormat = "0";
            }
            else if (e.Item == pivotPlant)
            {
                TheField = "PlantXP";
                FieldFormat = "0";
            }
            else if (e.Item == pivotPlow)
            {
                TheField = "PlowXP";
                FieldFormat = "0";
            }
            else if (e.Item == pivotHarvest)
            {
                TheField = "HarvestXP";
                FieldFormat = "0";
            }
            else if (e.Item == pivotMill)
            {
                TheField = "MillXP";
                FieldFormat = "0";
            }
        ...
        private void wcGetLBData_DownloadStringCompleted(object sender, DownloadStringCompletedEventArgs e)
        {
            try
            {
                ...
                if (TheField == "ExploreXP")
                {
                    listboxLBExplore.ItemsSource = tlistboxLeaderBoard;
                }
                else if (TheField == "PlantXP")
                {
                    listboxLBPlant.ItemsSource = tlistboxLeaderBoard;
                }
                else if (TheField == "PlowXP")
                {
                    listboxLBPlow.ItemsSource = tlistboxLeaderBoard;
                }
                else if (TheField == "HarvestXP")
                {
                    listboxLBHarvest.ItemsSource = tlistboxLeaderBoard;
                }
                else if (TheField == "MillXP")
                {
                    listboxLBMill.ItemsSource = tlistboxLeaderBoard;
                }
